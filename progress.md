...
## Log
- **2026-06-25** — Fixed scrollable timer picker initial alignment bug: wrapped the initial column scroll logic `scrollToSel` in a `setTimeout` with 0ms delay when opening the time picker panel. This ensures that the browser has computed the layout and scrollHeight of the columns, preventing scroll position settings from being ignored/reset and avoiding race conditions with scroll-snap alignment that automatically shifted hour and minute values to 1.
